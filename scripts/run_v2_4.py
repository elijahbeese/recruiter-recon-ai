...
run_v2_4.py
-----------
Recruiter Recon AI — Pipeline Orchestrator v2.4

Steps:
  1. Build candidate profile from resume (AI)
  2. Discover jobs (v2.4 — expanded private sector, 70+ threshold, dedup)
...
def main() -> None:
    ...
    print(f"[run_v2_4] Resume found: {resume_path}")

    run_step("Step 1 of 3 — Building candidate profile", "scripts/build_profile_v2_0.py")
    run_step("Step 2 of 3 — Discovering jobs (v2.4)", "scripts/discover_jobs_v2_4.py")
    ...
    print(f"\n[run_v2_4] Copying {discovered} → input_jobs.csv")
    shutil.copy(discovered, "input_jobs.csv")

    run_step("Step 3 of 3 — Enriching jobs (v2.0)", "scripts/recruiter_recon_v2_0.py")
